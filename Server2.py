...
def onehot_encoding(test, list, col):
    # kindNum을 원핫 인코딩
...
def index():
    ...
    # data = [[test[:, 0], test[:, 1], test[:, 2], test[:, 3:181], test[:, 181:184], test[:, 184:187]], ]
    data = [[onehot_encoding(test, kindCd, test.kindNum), onehot_encoding(test, neuter_list, test.neuterYn), onehot_encoding(test, sex_list, test.sexCd), weight, notice, age], ]
